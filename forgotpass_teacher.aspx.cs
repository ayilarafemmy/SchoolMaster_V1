...
public partial class pl : System.Web.UI.Page
{
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string tiwa = "Update D_Staff set Password='"+ TextBox3.Text+ "' where email='"+ Label5.Text+ "'";
            insertRecord(tiwa);
            int joe1 = int.Parse(ServerPort);
            string StaffMail = "" + Label2.Text + "" + Environment.NewLine + "Dear " + Label3.Text + "," + Environment.NewLine + "" + Environment.NewLine + "You have Successfully done a Password reset on " + Institution + " App" + Environment.NewLine + " " + Environment.NewLine + " New Password is: " + TextBox3.Text + " ";
            SendMail(Label5.Text, StaffMail, "Password Request - Reset Done", ServerName, SenderEmail, Institution, Mail_Password, joe1);


            MessageBox("Password Reset Done Successfully");
            HtmlMeta meta = new HtmlMeta();
            meta.HttpEquiv = "Refresh";
            meta.Content = "0;url=teacher.aspx";
        ...
        protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            con.Open();
            cmd.CommandText = "select * from D_Staff where Email='" + TextBox1.Text + "'";
            cmd.Connection = con;
            ...
            {
                Label2.Text = ds.Tables[0].Rows[0]["StaffID"].ToString();
                Label3.Text = ds.Tables[0].Rows[0]["FirstName"].ToString();
                Label4.Text = ds.Tables[0].Rows[0]["LastName"].ToString();
                Label5.Text = ds.Tables[0].Rows[0]["Email"].ToString();
                Random rd = new Random();
                Label6.Text = rd.Next(10000, 99999).ToString();

                //send mail
                if (sendmail == "Yes")
                {
                    int joe = int.Parse(ServerPort);
                    string StaffMail = ""+ Label2.Text + ""+Environment.NewLine+"Dear " + Label3.Text + "," + Environment.NewLine + "" + Environment.NewLine + "You have Successfully initiated a Password reset on " + Institution + " App" + Environment.NewLine + " " + Environment.NewLine + " Code is: "+ Label6.Text + " ";
                    SendMail(Label5.Text, StaffMail, "Password Request - Reset Code", ServerName, SenderEmail, Institution, Mail_Password, joe);
                }
                Label1.Visible = true;
                Label1.ForeColor = Color.Green;
                Label1.Text = "Code sent to your email address: "+TextBox1.Text+"";
                Panel1.Visible = true;
            ...
            else
            {
                ...
                HtmlMeta meta = new HtmlMeta();
                meta.HttpEquiv = "Refresh";
                meta.Content = "1;url=  forgotpass_teacher.aspx";
                this.Page.Controls.Add(meta);
            }
