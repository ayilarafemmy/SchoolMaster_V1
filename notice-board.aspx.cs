...
public partial class Admin_Students : System.Web.UI.Page
{
    ...
    {
        if (DropDownList5.SelectedItem.Text == "Select Recipient(s)")
        {
            MessageBox("Kindly Select Recipient(s)");
        ...
        {
            MessageBox("Kindly provide Message");
            return;
        }
        if (DropDownList5.SelectedItem.Text == "All Parents")
        {
            //get all parents email
            string strt = ConfigurationManager.ConnectionStrings["SchoolMaster"].ConnectionString;
            SqlConnection con = new SqlConnection(strt);
            con.Open();
            SqlCommand cmd = new SqlCommand("select email,Phone from D_Parents", con);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (sendmail == "Yes")
                {
                    ...
                    SendMail(email, TextBox3.Text, "School", ServerName, SenderEmail, Institution, Mail_Password, joe);

                    string quee = "insert into D_Logs (Activity, Details, Done_By,Capturetime) values ('Bill Sent', '"+TextBox3.Text+"', '" + Session["StaffID"].ToString() + "','" + DateTime.Now.ToString("dd/MM/yyyy") + "')";
                    insertRecord(quee);
        ...
        if (DropDownList5.SelectedItem.Text == "All Staff")
        {
            //get all parents email
            string strt = ConfigurationManager.ConnectionStrings["SchoolMaster"].ConnectionString;
            SqlConnection con = new SqlConnection(strt);
            con.Open();
            SqlCommand cmd = new SqlCommand("select email,Phone from D_Staff", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (sendmail == "Yes")
                {
                    ...
                    string quee = "insert into D_Logs (Activity, Details, Done_By,Capturetime) values ('Staff Business', '" + TextBox3.Text + "', '" + Session["StaffID"].ToString() + "','" + DateTime.Now.ToString("dd/MM/yyyy") + "')";
                    insertRecord(quee);
    ...
    protected void DropDownList4_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            DropDownList3.Enabled = true;
            string query1 = "Select RoutesID,Route from D_Routes";
            BindGrid1(query1, "Route", "RoutesID");
        }
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        if (DropDownList5.SelectedItem.Text == "All Parents")
        {
            ...
            HtmlMeta meta = new HtmlMeta();
            meta.HttpEquiv = "Refresh";
            meta.Content = "0;url=notice-board.aspx";
            this.Page.Controls.Add(meta);
        }
        if (DropDownList5.SelectedItem.Text == "All Staff")
        {
