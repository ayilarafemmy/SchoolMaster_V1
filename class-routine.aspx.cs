...
public partial class all_subject : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            if (!IsPostBack)
        {
            ...
            string query = "select a.PID,a.Session,a.Class,b.Subject,Day,Period,Concat(c.FirstName,' ',c.LastName) as Name,a.SessionID from D_Period a, D_Subjects b, D_Staff c where a.Subject = b.SubjectID and a.Teacher = c.StaffID;";
            GridviewBind(query);
        }
        con.Close();
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        conx.Open();
        cmdx.CommandText = "select subject from D_Period where Subject='" + DropDownList1.SelectedValue + "' and SessionID='"+ Label1.Text + "' and Class='"+Label3.Text+"' and Period='"+DropDownList3.SelectedItem.Text+ "' and Day='"+DropDownList4.SelectedItem.Text+"'";
        cmdx.Connection = conx;
        sdax.SelectCommand = cmdx;
        sdax.Fill(dsx, "D_Period");

        if (dsx.Tables[0].Rows.Count >= 1)
        {
            string myStringVariable = "Subject : " + DropDownList1.SelectedItem.Text + " already exists for this period and day this term!";
        ...
        if (DropDownList4.SelectedItem.Text == "Select Day")
        {
            MessageBox("Kindly Select Day");
            return;
        }
        string query = "insert into D_Period (SessionID,Session,Subject,Class,Day,Period,CreateDate,CreatedBy,Teacher,ClassID) values ('"+Label1.Text+"','"+TextBox1.Text+"','"+DropDownList1.SelectedValue+"','"+Label3.Text+"','"+DropDownList4.SelectedItem.Text+"','"+DropDownList3.SelectedItem.Text+"','"+DateTime.Now.ToString("dd/MM/yyyy")+"','"+Session["StaffID"].ToString()+"','"+DropDownList2.SelectedValue+"','"+Label4.Text+"')";
        insertRecord(query);

        MessageBox("Period Captured Successfully");
        HtmlMeta meta = new HtmlMeta();
        meta.HttpEquiv = "Refresh";
        meta.Content = "0;url=class-routine.aspx";
    ...
    protected void BindGrid2()
    {
        ...
        {
            con.Open();
            string com = "Select PeriodID,Timing from D_PeriodPeriod";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
            DropDownList3.DataSource = dt;
            DropDownList3.DataBind();
            DropDownList3.DataTextField = "Timing";
            DropDownList3.DataValueField = "Periodid";
            DropDownList3.DataBind();
